...
2. 2. **Sélectionne** l'outil approprié (`ai_dna_translator` ou `ai_species_identifier`).
   3. 3. **Exécute** le code Python/Biopython sous-jacent.
      4. 4. **Synthétise** le résultat déterministe sous la forme d'un "Rapport de Laboratoire / Scène de crime" complet.
        
         5. ## ⚙️ Architecture & Stack Technique
        
         6. - **Le Cerveau (LLM)** : `Qwen/Qwen2.5-Coder-32B-Instruct` (via l'API d'inférence Hugging Face). Choisi pour ses excellentes capacités natives de "Function Calling" (Appel de fonctions).
                                   ...
                                   - - **Prompting Système Strict** : L'Agent a pour instruction explicite de **ne jamais** deviner ou inventer une espèce si l'outil renvoie un résultat négatif ou si la séquence fait moins de 12 nucléotides.
                                     ...
                                     **Thierry Maesen**
                                     ...
                                     - - **Concept** : Démonstration de l'architecture *Agentic Workflow* et *AI for Science*.
                                       ...
                                       # 🧬 Gene-Agent: Autonomous Genomic AI (Agentic Workflow)
                                       ...
                                       **Gene-Agent** is an autonomous Artificial Intelligence system designed for bioinformatics. Unlike traditional chatbots or RAG systems, this project uses an **Agentic Workflow**, enabling the language model (LLM) to reason autonomously, select and execute custom Python tools to analyze DNA sequences.
                                       ...
                                       ## 🚀 The Concept: AI for Science

                                       This project demonstrates how Large Language Models (LLMs) can be equipped with deterministic scientific tools to avoid "hallucinations" and deliver accurate, verifiable results. When a user submits a DNA sequence, the Agent:

                                       1. **Reasons** about the request (Is it a translation? An identification?).
                                       2. 2. **Selects** the appropriate tool (`ai_dna_translator` or `ai_species_identifier`).
                                          3. 3. **Executes** the underlying Python/Biopython code.
                                             4. 4. **Synthesizes** the deterministic result into a comprehensive "Laboratory / Crime Scene Report".
                                               
                                                5. ## ⚙️ Architecture & Tech Stack
                                               
                                                6. - **The Brain (LLM)**: `Qwen/Qwen2.5-Coder-32B-Instruct` (via Hugging Face Inference API). Chosen for its excellent native "Function Calling" capabilities.
                                                   - - **The Orchestrator**: `smolagents` (Hugging Face). An ultra-lightweight and secure framework for building autonomous agents.
                                                     - - **The Tools**: Custom Python functions using `Biopython` to ensure absolute scientific rigor and sanitize inputs (via Regex).
                                                       - - **The Interface**: `Streamlit` for a real-time reactive user experience, allowing visualization of the Agent's reasoning process ("Thought Process").
                                                        
                                                         - ### ⚠️ Note on the Database (Demo Mode)
                                                        
                                                         - To ensure sub-second response times for the user demonstration, the `ai_species_identifier` tool is currently connected to a highly optimized local dictionary containing famous genetic sequences (Covid-19, T-Rex, Tardigrade, etc.).
                                                        
                                                         - *In a real production environment, this tool would be replaced by a query to the public US NCBI BLAST API (which typically requires 1 to 5 minutes of processing per request).*
                                                        
                                                         - ## 🛠️ Local Installation
                                                        
                                                         - 1. Clone the repository:
                                                           2. ```bash
                                                              git clone https://github.com/YOUR_USERNAME/Gene-Agent.git
                                                              cd Gene-Agent
                                                              ```
                                                              ...
                                                              3. ```bash
                                                                 ...
                                                                 4. ```bash
                                                                    ...
                                                                    *(Note: You will need a free Hugging Face API key with 'Read' permissions to run the agent).*

                                                                    ## 🧪 Quick Test Sequences

                                                                    Try these sequences in the application to see the Agent in action:

                                                                    - `ATGGCCCTGTGGATGCGCCTCCTG` (Human Insulin)
                                                                    - - `ATGTTTGTTTTTCTTGTTTTA` (SARS-CoV-2 Spike Protein)
                                                                      - - `ATGGGATATACTAGTATGGGA` (Estimated T-Rex Collagen)
                                                                       
                                                                        - ## 🔒 Security & Anti-Hallucination
                                                                       
                                                                        - The system is built with strict safeguards:
                                                                       
                                                                        - - **Input Sanitization**: Python tools validate inputs via Regex (`^[ACTG]+$`) before processing to prevent any prompt injection.
                                                                          - - **Strict System Prompting**: The Agent is explicitly instructed to **never** guess or invent a species if the tool returns a negative result or if the sequence is shorter than 12 nucleotides.
                                                                           
                                                                            - ---

                                                                            *Created to demonstrate the application of Agentic AI to the field of Bioinformatics.*
                                                                            ...
                                                                            **Thierry Maesen**
                                                                            ...
                                                                            - **Project**: [Gene-Agent GitHub Repository](https://github.com/thierrymaesen/Gene-Agent)
                                                                            - - **Concept**: Demonstration of *Agentic Workflow* and *AI for Science* architecture.
                                                                             
                                                                              - *This project was designed to illustrate the creation of secure, hallucination-resilient Autonomous Agents, deployed through a professional CI/CD pipeline.*
